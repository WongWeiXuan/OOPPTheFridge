...
public class CharityFoodDonationGoogleMapModel implements DirectionsServiceCallback {
	...
	public CharityFoodDonationGoogleMapModel(GoogleMapView mapView, VBox vboxInScroll){
	...
	public void setTextLabels(DirectionsService directionsService) throws FileNotFoundException{
		...
		for(int i = 0; i < placeList.size(); i++){
			...
			Label PlaceNameLbl = new Label(placeList.get(i).getName());
			Label DistanceLbl = new Label(String.valueOf(placeList.get(i).getDistance()));
			Label EtaLbl = new Label(String.valueOf(placeList.get(i).getDuration()));
			...
			stackPane.setOnMouseClicked((MouseEvent event) -> {
		        ...
		        directionsDisplay.setMap(mapView.createMap(mapOptions));
